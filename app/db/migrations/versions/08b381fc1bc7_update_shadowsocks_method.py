...
def upgrade() -> None:
    ...
    stmt = sa.select(proxies_table.c.id, proxies_table.c.settings).where(proxies_table.c.type == "Shadowsocks")
    ...
    for pid, settings in result:
        ...
        if settings.get("method") == "chacha20-poly1305":
            ...
            update_stmt = (
                sa.update(proxies_table).where(proxies_table.c.id == pid).values(settings=json.dumps(new_settings))
            )
