...
def upgrade():
    ...
    with op.batch_alter_table("hosts") as batch_op:
        batch_op.alter_column("host", existing_type=sa.String(length=256), type_=sa.String(length=1000), nullable=True)
        batch_op.alter_column("sni", existing_type=sa.String(length=256), type_=sa.String(length=1000), nullable=True)


def downgrade():
    # Revert the column lengths back to their original size
    with op.batch_alter_table("hosts") as batch_op:
        batch_op.alter_column("host", existing_type=sa.String(length=1000), type_=sa.String(length=256), nullable=True)
